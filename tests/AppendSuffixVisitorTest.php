...
class AppendSuffixVisitorTest extends AbstractTestCase
{
    ...
    public function it_should_traverse_and_append_a_suffix_to_classes_imports_and_fqcn_calls()
    {
        ...
        $code .= '{'.PHP_EOL.'}'.PHP_EOL;
        $code .= 'class AppendSuffix extends AppendSuffixAbstract implements AppendSuffixInterface, Baz, Beep\Beeep'.PHP_EOL;
        $code .= '{'.PHP_EOL;
        $code .= '    use AppendSuffixTrait;'.PHP_EOL;
        $code .= '    use Baz\BazTrait;'.PHP_EOL;
        $code .= '    use \Outside\Tr;'.PHP_EOL;
        $code .= '    public function add(Bar $bar, SomeClass $z, \Fqcn $f)'.PHP_EOL;
        $code .= '    {'.PHP_EOL;
        ...
        $expected .= 'use User_123 as User;'.PHP_EOL;
        ...
        $expected .= '{'.PHP_EOL.'}'.PHP_EOL;
        $expected .= 'class AppendSuffix_123 extends AppendSuffixAbstract_123 implements AppendSuffixInterface_123, Baz, Beep\Beeep_123'.PHP_EOL;
        $expected .= '{'.PHP_EOL;
        $expected .= '    use AppendSuffixTrait_123;'.PHP_EOL;
        $expected .= '    use Baz\BazTrait_123;'.PHP_EOL;
        $expected .= '    use \Outside\Tr_123;'.PHP_EOL;
        $expected .= '    public function add(Bar $bar, SomeClass_123 $z, \Fqcn_123 $f)'.PHP_EOL;
        $expected .= '    {'.PHP_EOL;
